...
- **Sorting**: Right-click a column header and open the **Sort** submenu to sort ascending or descending — **Display Sorted** reorders just the view and leaves the file alone, **Sort Rows** reorders the file itself. Values are typed automatically, so timestamps sort chronologically and currency amounts like `$1,200.00` sort by value rather than as text. If editing a cell moves its row within the sorted view, a dismissable notice offers to jump to where the row went
